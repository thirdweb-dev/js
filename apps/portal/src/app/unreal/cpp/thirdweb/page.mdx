...
- **`AssignRetryResult(bCanRetry, Output, bErrorOnlyResult)`**: Assign's result with retry and then frees the
  underlying pointer
...
- **`create_in_app_wallet(client_id, bundle_id, secret_key, email, storage_directory_path, oauth_provider)`**: Creates
  an in-app wallet.
