...
Once authenticated, the auth endpoints will return the wallet address and a JWT token for usage with the rest of the API.

<OpenApiEndpoint path="/v1/wallets/user/code" method="POST" />
<OpenApiEndpoint path="/v1/wallets/user/code/verify" method="POST" />
