...
export async function fetchInAppWalletMetadataFromThirdweb(
	...
	if (args.queryBy === "externalWaletAddress") {
		url.searchParams.set("queryBy", "walletAddress");
		url.searchParams.set("externalWaletAddress", args.externalWaletAddress);
	}
	...
	const data = (await resp.json()) as {
		...
		linkedAccounts: {
			type: string;
			details: { email: string } | { phone: string } | { walletAddress: string };
		}[];
	}[];
	return data;
}
