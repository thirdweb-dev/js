...
export async function listVaultServerWallets(params: {
  ...
  page?: number;
  pageSize?: number;
}): Promise<VaultActionResult<VaultServerWalletPage>> {
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to list server wallets"),
      ok: false,
    };
  }
...
> {
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to create server wallet"),
      ok: false,
    };
  }
...
export async function setVaultProjectWallet(params: {
  project: ProjectRef;
  address: string;
}): Promise<VaultActionResult<{ projectWalletAddress: string }>> {
  const res = await apiServerProxy<{
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to update project wallet"),
      ok: false,
    };
  }
...
export async function createVaultServiceAccount(params: {
...
}): Promise<VaultActionResult<CreateVaultServiceAccountResult>> {
  const res = await apiServerProxy<{
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to create vault"),
      ok: false,
    };
  }
...
export async function rotateVaultServiceAccount(params: {
  ...
  mode?: "managed" | "ejected";
  projectSecretKey?: string;
}): Promise<VaultActionResult<RotateVaultServiceAccountResult>> {
  const res = await apiServerProxy<{
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to rotate admin key"),
      ok: false,
    };
  }
...
export async function listVaultAccessTokens(params: {
  ...
  page?: number;
  pageSize?: number;
}): Promise<VaultActionResult<VaultAccessTokenPage>> {
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to list access tokens"),
      ok: false,
    };
  }
...
export async function createVaultAccessToken(params: {
  ...
  VaultActionResult<{ id: string; accessToken: string; purpose: string }>
> {
  const res = await apiServerProxy<{
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to create access token"),
      ok: false,
    };
  }
...
export async function revokeVaultAccessToken(params: {
  ...
  if (!res.ok) {
    return {
      error: errorMessage(res.error, "Failed to revoke access token"),
      ok: false,
    };
  }
