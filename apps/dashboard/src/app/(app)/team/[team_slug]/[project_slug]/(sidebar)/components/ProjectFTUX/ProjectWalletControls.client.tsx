...
function SendProjectWalletModal(props: {
  ...
  const credentialStorageKey = useMemo(() => {
    const baseKey = projectId ?? publishableKey;
    return `thirdweb-dashboard:project-wallet-credentials:${baseKey}`;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cached?.secretKey) {
        form.setValue("secretKey", cached.secretKey, {
          shouldDirty: false,
          shouldValidate: true,
        });
      }

      if (!isManagedVault && cached?.vaultAccessToken) {
        form.setValue("vaultAccessToken", cached.vaultAccessToken, {
          shouldDirty: false,
          shouldValidate: false,
        });
      }
    } catch (error) {
      console.error("Failed to restore cached wallet credentials", error);
    }
  ...
  useEffect(() => {
    ...
    const subscription = form.watch((values, { name }) => {
      if (!name || (name !== "secretKey" && name !== "vaultAccessToken")) {
        return;
      }

      const secretKeyValue = values.secretKey?.trim() ?? "";
      const vaultTokenValue = values.vaultAccessToken?.trim() ?? "";

      if (!secretKeyValue && !vaultTokenValue) {
      ...
      try {
        ...
        if (secretKeyValue) {
          payload.secretKey = secretKeyValue;
        }

        if (!isManagedVault && vaultTokenValue) {
          payload.vaultAccessToken = vaultTokenValue;
        }
        ...
        console.error("Failed to cache wallet credentials", error);
      }
    });

    return () => subscription.unsubscribe();
  }, [form, credentialStorageKey, isManagedVault]);
