...
import type { BinanceConnector as BinanceWalletConnectorType } from "../connectors/binance-wallet";
import { walletIds } from "../constants/walletIds";
import { TW_WC_PROJECT_ID } from "../constants/wc";
import { getInjectedBinanceProvider } from "../connectors/binance-wallet/getInjectedBinanceProvider";

export type BinanceWalletAdditionalOptions = {
  /**
   * Whether to open the default Wallet Connect QR code Modal for connecting to Binance app on mobile if Binance wallet is not injected when calling connect().
   */
  qrcode?: boolean;
...
export type BinanceWalletOptions = WalletOptions<BinanceWalletAdditionalOptions>;
 ...
 * @wallet
 */
export class BinanceWallet extends AbstractClientWallet<BinanceWalletAdditionalOptions> {
  /**
   * @internal
   */
  connector?: Connector;
   ...
   * @internal
   */
  BinanceWalletConnector?: BinanceWalletConnectorType;
  /**
   * @internal
   */
  isInjected: boolean;
  ...
  public get walletName() {
    return "Binance Wallet" as const;
  }
   ...
   *
   */
  constructor(options: BinanceWalletOptions) {
  ...
  protected async getConnector(): Promise<Connector> {
    if (!this.connector) {
      // if Binance wallet is injected, use the injected connector
      // otherwise, use the wallet connect connector for using the Binance app on mobile via QR code scan

      if (this.isInjected) {
        // import the connector dynamically
        const { BinanceConnector } = await import(
          "../connectors/binance-wallet"
        );
        this.BinanceWalletConnector = new BinanceConnector({
          chains: this.chains,
        ...
        this.connector = new WagmiAdapter(this.BinanceWalletConnector);
      } else {
   ...
   * You can use this method to display a QR code. The user can scan this QR code using the Binance Wallet mobile app to connect to your dapp.
   *
   * @example
   * ```typescript
