...
export class MerkleTree extends Base {
  ...
  constructor(leaves: any[], hashFn: any, options: Options = {}) {
    ...
    if (options.fillDefaultHash) {
      if (typeof options.fillDefaultHash === "function") {
        ...
        Buffer.isBuffer(options.fillDefaultHash) ||
  ...
  getProof(
    leaf: Buffer | string,
    index?: number,
  ): { position: "left" | "right"; data: Buffer }[] {
  ...
  getProofFlags(leaves: any[], proofs: Buffer[] | string[]): boolean[] {
    ...
    const _proofs: Buffer[] = (proofs as any[]).map((item) =>
      this.bufferify(item),
    );
  ...
  getLayersAsObject(): any {
    const layers: any[] = this.getLayers().map((layer: any) =>
      layer.map((value: any) => this.bufferToHex(value, false)),
    );
    const objs = [];
