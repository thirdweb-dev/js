...
export function generateSnippets(ecosystem, hooksConfig) {
  const tsdocParser = new TSDocParser();

  const json = JSON.parse(
    fs.readFileSync(
      `${process.cwd()}/temp-${ecosystem}/react.api.json`,
      "utf8",
    ),
  );
  ...
  const hooks = json.members[0].members.filter(
    (m) => m.kind === "Function" && Object.keys(hooksConfig).includes(m.name),
  );
  ...
  const moduleMap = hooks.reduce((acc, m) => {
    ...
    const contractSubhooks = json.members[0].members.filter(
      (f) => f.kind === "Function" && hooksConfig[m.name].includes(f.name),
    );
...
function generateEvmSnippets() {
  ...
  generateSnippets("evm", HOOKS_CONFIG);
}

function generateSolanaSnippets() {
  ...
  const PROGRAM_HOOKS = ["useProgramAccountType", "useProgramMetadata"];

  const HOOKS_CONFIG = {
    useNFTCollection: [...NFT_HOOKS],
    useNFTDrop: [...NFT_HOOKS, ...DROP_HOOKS],
    useToken: [...TOKEN_HOOKS],
    useProgram: [...PROGRAM_HOOKS],
  };

  generateSnippets("solana", HOOKS_CONFIG);
}

generateEvmSnippets();
generateSolanaSnippets();
