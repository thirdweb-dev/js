...
import { forceDeploySmartAccount } from "./lib/signing.js";
...
describe.runIf(process.env.TW_SECRET_KEY).sequential(
  ...
  () => {
    ...
    it("should force deployment on signature if eagerDeployment is true", async () => {
      const eagerDeploymentWallet = smartWallet({
        chain,
        gasless: true,
        eagerDeployment: true,
      });
      const smartAccount = await eagerDeploymentWallet.connect({
        client: TEST_CLIENT,
        personalAccount,
      });
