...
const FRAME_HTTP_URL = "http://127.0.0.1:1248";

function getFrameProvider(): Ethereum {
  let id = 0;
  ...
  return {
    request: async (params) => {
      ...
      const res = await fetch(FRAME_HTTP_URL, {
        method: "POST",
        body: request,
        headers: {
 ...
 * @internal
 */
export async function connectFrameWallet(
 ...
 * @internal
 */
export async function autoConnectFrameWallet(
  emitter: WalletEmitter<InjectedSupportedWalletIds>,
): Promise<ReturnType<typeof onConnect>> {
  const provider = getFrameProvider();
