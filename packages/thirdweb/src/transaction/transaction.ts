...
import type { ContractOptions, ThirdwebContract } from "../contract/index.js";
...
export type Transaction<abiFn extends AbiFunction> = {
  inputs: TransactionOptions<string, abiFn> & ContractOptions;
...
>(
  ...
  const mergedOptions = { ...options, ...client } as TransactionOptions<
    method,
    abi
  > &
    ContractOptions;
  return {
    inputs: mergedOptions,
    client: client,
    abi: memoizePromise(async () => {
      ...
      const abi = await resolveAbi(mergedOptions);
