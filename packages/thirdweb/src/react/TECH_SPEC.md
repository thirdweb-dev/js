...
4. **Tier-3 Flows** – `<FundWallet />`, `<DirectPayment />`, `<TransactionPayment />`.
...
| ------------------------ | ------------------ | ------------------------------------------------------------------------- |
| `<FundWallet />`         | `"fund_wallet"`    | Simplest flow; destination = connected wallet.                            |
...
Each exports both named component **and** factory: `createFundWalletFlow(config)`. Factories are helpful for test stubs.
