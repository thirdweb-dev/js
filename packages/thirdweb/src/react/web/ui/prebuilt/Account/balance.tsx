...
export interface AccountBalanceProps
   ...
   */
  queryOptions?: Omit<UseQueryOptions<AccountBalanceInfo>, "queryFn">;

  /**
   * Show the token balance in a supported fiat currency (e.g "USD")
   */
  showInFiat?: SupportedFiatCurrency;
}
 ...
 *
 * ```tsx
 * import type { AccountInfoBalance } from "thirdweb/react";
...
export async function loadAccountBalance(props: {
  ...
  const { chain, client, address, tokenAddress, showInFiat } = props;
  if (!chain) {
  ...
  if (showInFiat) {
    ...
    if (fiatData === undefined) {
      throw new Error(
        `Failed to resolve fiat value for token: ${tokenAddress} on chainId: ${chain.id}`,
      );
    }
    return {
