...
export function useBridgePrepare(params: UseBridgePrepareParams) {
  ...
  return useQuery({
    queryKey: ["bridge-prepare", type, prepareParams],
    queryFn: async () => {
      switch (type) {
        case "buy":
          return Bridge.Buy.prepare(prepareParams as BuyPrepare.Options);
        case "sell":
          return Bridge.Sell.prepare(prepareParams as SellPrepare.Options);
        case "transfer":
          return Bridge.Transfer.prepare(
            prepareParams as TransferPrepare.Options,
          );
        case "onramp":
          return Bridge.Onramp.prepare(prepareParams as OnrampPrepare.Options);
        default:
