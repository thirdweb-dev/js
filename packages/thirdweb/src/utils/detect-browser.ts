...
// a cache for the result of the detection
let detectResult: ReturnType<typeof detect> | null = null;
export function detect(
  ...
  | ReactNativeInfo
  | null {
  // keep the last result around, as there is no reason to call it again
  if (detectResult) {
    return detectResult;
  }
  if (!!userAgent) {
    detectResult = parseUserAgent(userAgent);
  } else if (
    typeof document === "undefined" &&
    typeof navigator !== "undefined" &&
    navigator.product === "ReactNative"
  ) {
    detectResult = new ReactNativeInfo();
  } else if (typeof navigator !== "undefined") {
    detectResult = parseUserAgent(navigator.userAgent);
  } else {
    detectResult = getNodeVersion();
  }
  return detectResult;
}
