...
export const FN_SELECTOR = "0x46904840" as const;
 ...
 * @example
 * ```ts
 * import { isFeeRecipientSupported } from "thirdweb/extensions/assets";
 * const supported = isFeeRecipientSupported(["0x..."]);
 * ```
 */
export function isFeeRecipientSupported(availableSelectors: string[]) {
  return detectMethod({
    availableSelectors,
 ...
 * @example
 * ```ts
 * import { decodeFeeRecipientResult } from "thirdweb/extensions/assets";
 * const result = decodeFeeRecipientResultResult("...");
 * ```
 */
export function decodeFeeRecipientResult(result: Hex) {
 ...
 * @example
 * ```ts
 * import { feeRecipient } from "thirdweb/extensions/assets";
 *
 ...
 * ```
 */
export async function feeRecipient(options: BaseTransactionOptions) {
  return readContract({
