 ...
 * @returns A promise that resolves to a boolean indicating if the `execute` method is supported.
 * @extension VOTE
 * @example
 * ```ts
 * import { isExecuteSupported } from "thirdweb/extensions/vote";
 *
 * const supported = await isExecuteSupported(contract);
 * ```
 */
export async function isExecuteSupported(contract: ThirdwebContract<any>) {
  return detectMethod({
    contract,
