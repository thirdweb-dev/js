 ...
 * @returns A promise that resolves to a boolean indicating if the `relay` method is supported.
 * @extension VOTE
 * @example
 * ```ts
 * import { isRelaySupported } from "thirdweb/extensions/vote";
 *
 * const supported = await isRelaySupported(contract);
 * ```
 */
export async function isRelaySupported(contract: ThirdwebContract<any>) {
  return detectMethod({
    contract,
