 ...
 * @returns A promise that resolves to a boolean indicating if the `proposals` method is supported.
 * @extension VOTE
 * @example
 * ```ts
 * import { isProposalsSupported } from "thirdweb/extensions/vote";
 *
 * const supported = await isProposalsSupported(contract);
 * ```
 */
export async function isProposalsSupported(contract: ThirdwebContract<any>) {
  return detectMethod({
    contract,
