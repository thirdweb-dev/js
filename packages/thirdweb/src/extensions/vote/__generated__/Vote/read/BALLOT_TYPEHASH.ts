 ...
 * @returns A promise that resolves to a boolean indicating if the `BALLOT_TYPEHASH` method is supported.
 * @extension VOTE
 * @example
 * ```ts
 * import { isBALLOT_TYPEHASHSupported } from "thirdweb/extensions/vote";
 *
 * const supported = await isBALLOT_TYPEHASHSupported(contract);
 * ```
 */
export async function isBALLOT_TYPEHASHSupported(
