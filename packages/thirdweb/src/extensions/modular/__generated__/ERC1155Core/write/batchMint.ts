 ...
 * @returns A promise that resolves to a boolean indicating if the `batchMint` method is supported.
 * @extension MODULAR
 * @example
 * ```ts
 * import { isBatchMintSupported } from "thirdweb/extensions/modular";
 *
 * const supported = await isBatchMintSupported(contract);
 * ```
 */
export async function isBatchMintSupported(contract: ThirdwebContract<any>) {
  return detectMethod({
    contract,
