...
export const FN_SELECTOR = "0x0c8d9cc7" as const;
const FN_INPUTS = [
  {
    type: "address",
    name: "poolRouter",
 ...
 * ```
 */
export function isSetPoolRouterSupported(availableSelectors: string[]) {
  return detectMethod({
    availableSelectors,
 ...
 * ```
 */
export function encodeSetPoolRouterParams(options: SetPoolRouterParams) {
 ...
 * ```
 */
export function encodeSetPoolRouter(options: SetPoolRouterParams) {
  // we do a "manual" concat here to avoid the overhead of the "concatHex" function
  // we can do this because we know the specific formats of the values
  return (FN_SELECTOR +
    encodeSetPoolRouterParams(options).slice(
      2,
...
export function setPoolRouter(
  ...
  return prepareContractCall({
    ...
    params: async () => {
      ...
      return [resolvedOptions.poolRouter] as const;
    },
