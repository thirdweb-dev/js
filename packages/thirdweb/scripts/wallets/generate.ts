...
const walletConnectSupportedWallets = uniqueWallets.filter((w) => {
  return w.mobile.universal || w.desktop.universal;
});

const injectedSupportedWallets = uniqueWallets.filter((w) => {
...
const walletImports = uniqueWallets
  .map(
    (wallet) =>
      `case "${wallet.id}": {
        return (
 ...
 * @internal
 */
export async function getWalletInfo<TImage extends boolean>(id: string, image?: TImage): Promise<[TImage] extends [true] ? string : WalletInfo> {
  switch (id) {
    ${walletImports}
    default: {
