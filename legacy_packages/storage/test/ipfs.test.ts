...
// these cannot run on forks because of the missing secret key env variable
describe.skipIf(!secretKey)(
  ...
  async () => {
    ...
    it("Should batch upload strings with names", async () => {
      ...
      expect(uris[0].endsWith("first"), `${uris[0]} does not end with 'first'`)
        .to.be.true;
      expect(
        uris[1].endsWith("second"),
        `${uris[1]} does not end with 'second'`,
      ).to.be.true;
    ...
    it("Should batch upload JSON objects", async () => {
      ...
      expect(uris[0].endsWith("0"), `${uris[0]} does not end with '0'`).to.be
        .true;
      expect(uris[1].endsWith("1"), `${uris[1]} does not end with '0.jpg'`).to
        .be.true;
    ...
    it("Should upload without directory if specified on function", async () => {
      ...
      expect(json.description).to.equal(
        "Uploading alone without a directory...",
      );
    });
